...
def align(arr, aln, fb=10,eb=10, delay=0):
    aln = aln+fb
...
class ECoGDataset(Dataset):
    ...
    def __init__(self, data, load_list=[], text_list=[], transform=None,only_hga=False,
                multi_hb_dirs=None, do_align=False, fb=10,eb=10, align_target=[], delay=0):
        super().__init__()
        self.data = data
        ...
        for entry in self.load_list:
            if entry =='ecog':
                ...
                if entry in self.align_target:
                    feature = align(feature, aln,self.fb,self.eb,self.delay)
