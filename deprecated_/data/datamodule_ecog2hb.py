...
class ECoGPhonemeDataset(Dataset):
    ...
    def __init__(self, data, transform=None, do_preprocess=True, decimate=4):
        super().__init__()
        self.data = data
...
class ECoGPhonemeDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        dataset = ECoGPhonemeDataset(data, transform=self.get_transform('train',**self.transform_config),
                                    do_preprocess=self.do_preprocess,decimate=self.decimate)
        loader = DataLoader(
            dataset,
    ...
    def get_transform(self,mode='train',
        ...
        if mode =='train':
            ...
            return transforms.Compose([jitter,  blackout, noise,
                                                 chan_noise, scale])
        else:
            jitter = Jitter((-1, 8), 
                            (0.5, 7.5),
