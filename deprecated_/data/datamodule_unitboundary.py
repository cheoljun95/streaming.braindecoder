...
class SpeechTextDataset(Dataset):
    ...
    def __init__(self, data, skip_text=False, max_len=None, label_sr=50, collapse=False):
        super().__init__()
        self.data = data
    ...
    def __getitem__(self,i):
        text,bd = self.data[i]
        bd = np.load(bd)
        unit = np.array(text.split(' ')).astype(int)
        if self.max_len is not None:
            unit = unit[:int(self.max_len*self.label_sr)]
            bd = bd[:int(self.max_len*self.label_sr)]
            
        if self.collapse:
            unit_ = [unit[0]]
            bd_ = [bd[0]]
            for u,b in zip(unit[1:], bd[1:]):
                if unit_[-1] == u:
                    bd_[-1] = max(b,bd[-1])
                else:
                    unit_.append(u)
            ...
            unit = np.array(unit_)
            bd = np.array(bd_)
        output = {'unit':torch.from_numpy(unit).long(),
                  'boundary': torch.from_numpy(bd).long()}
    ...
    def collate(batch):
        data = {}
        data['units'] = nn.utils.rnn.pad_sequence([d['unit'] for d in batch], batch_first=True, padding_value=100)
        data['unit_lens'] = torch.tensor([len(d['unit']) for d in batch])
        data['boundaries'] = nn.utils.rnn.pad_sequence([d['boundary'] for d in batch], batch_first=True, padding_value=0)
        data['boundary_lens'] = torch.tensor([len(d['boundary']) for d in batch])
        return data
...
class SpeechTextDataModule(pl.LightningDataModule):
    ...
    def _load_data(self, split):
        ...
        for split_name in split_names:
            ...
            for tag,text in zip(tags, texts):
                len_, tag=tag.split(' ')
                if not (self.bd_dir/f'{tag}.npy').exists():
                    continue
                ...
                data.append([text, self.bd_dir/f'{tag}.npy'])
        return data
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        dataset = SpeechTextDataset(data,max_len=self.max_len,label_sr=self.label_sr,collapse=self.collapse)
        loader = DataLoader(
            dataset,
